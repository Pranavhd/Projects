...
##training_tweets = dfannon['text'].tolist()
...
for tweet in testing_tweets:
    ...
    if topic_name == 'bag':
        list_text_Bag.append(tweet)
    if topic_name == 'flight':
        list_text_Flight.append(tweet)
    if topic_name == 'service':
        list_text_Service.append(tweet)
print(dict_prediction)

df4['text'] = pd.Series(list_text_Bag)
df4.to_csv('Output_Bag.csv')
df5['text'] = pd.Series(list_text_Flight)
df5.to_csv('Output_Flight.csv')
df6['text'] = pd.Series(list_text_Service)
df6.to_csv('Output_Service.csv')

print(dict_prediction)
print(len(list_text_Bag) ,len(list_text_Flight) ,len(list_text_Service))
ldamodel.save('final_model_output_input_tweets')
